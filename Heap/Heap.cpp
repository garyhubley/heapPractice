...
void Heap::SiftDown(unsigned node) {
	...
	while ((leftChild = GetLeftChild(node)) < _size) {
		...
		if ((rightChild = GetRightChild(node)) < _size && _data[rightChild] > _data[leftChild]) {
...
unsigned Heap::GetLeftChild(unsigned node) {
	return node * 2 + 1;
}

unsigned Heap::GetRightChild(unsigned node) {
	return node * 2 + 2;
}
