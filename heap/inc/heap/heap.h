...
class Heap {
   private:
    ...
    // delete an arbitrary node(followed by moving last node and sifting to
    // maintain heap)
   ...
   public:
    ...
    static void printVersion(std::ostream &out) {
        out << "garysHeap Version " << garysHeap_VERSION_MAJOR << "."
            << garysHeap_VERSION_MINOR << std::endl;
    }

    Heap &operator=(const Heap &rhs);

    // determine if a value is inside the heap
    bool Find(int value);

    // find a maximum item of a max - heap, or a minimum item of a min -
    // heap, respectively(a.k.a.peek)
    int FindMax();

    // adding a new key to the heap(a.k.a., push[4])
    void Insert(int value);

    // returns the node of maximum value from a max heap[or minimum value
    // from a min heap] after removing it from the heap(a.k.a., pop[5])
    int ExtractMax();
    ...
    // since only need to balance once, not twice, and appropriate for fixed
    // - size heaps.[6]
